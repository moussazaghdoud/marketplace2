    ...
    function renderStep2() {
        ...
        card.innerHTML =
            ...
            '<div class="mb-3">' +
                ...
                '<div class="relative">' +
                    '<input type="password" id="ob-password" placeholder="Min 12 chars, upper+lower+digit+special" class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-200 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm">' +
                    '<button type="button" id="ob-pwd-toggle" class="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded" tabindex="-1">' + eyeIcon(false) + '</button>' +
                '</div>' +
            '</div>' +
            '<div class="mb-4">' +
                '<label class="block text-xs font-medium text-gray-600 mb-1">Confirm password</label>' +
                '<div class="relative">' +
                    '<input type="password" id="ob-password2" placeholder="Repeat password" class="w-full px-3 py-2.5 pr-10 rounded-lg border border-gray-200 focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none text-sm">' +
                    '<button type="button" id="ob-pwd2-toggle" class="absolute right-2.5 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded" tabindex="-1">' + eyeIcon(false) + '</button>' +
                '</div>' +
            '</div>' +
            '<p id="ob-error" class="text-red-500 text-xs mb-3 ' + (state.error ? '' : 'hidden') + '">' + esc(state.error) + '</p>' +
            '<button id="ob-submit" class="w-full py-3 rounded-lg bg-brand-500 text-white font-medium text-sm hover:bg-brand-600 transition-colors" ' + (state.loading ? 'disabled' : '') + '>' +
        ...
        $('ob-pwd-toggle').addEventListener('click', function () { togglePassword('ob-password', 'ob-pwd-toggle'); });
        $('ob-pwd2-toggle').addEventListener('click', function () { togglePassword('ob-password2', 'ob-pwd2-toggle'); });
